...
- `web`: restored the public demo-case flow by giving `demo-task-id` a local prefilled draft path instead of depending on a server-side task record that may not exist on a fresh deployment.
- `docs`: added `docs/ACCEPTANCE_CHECKLIST.md` and `docs/OPERATIONS_RUNBOOK.md` to capture the final go-live checklist and recurring Alibaba Cloud/Nginx operations notes.
...
| 2026-05-24 | Public demo flow | “查看演示案例” opened the demo route but showed an empty form on fresh deployments | The homepage linked to `demo-task-id`, while the page hook only tried to fetch a server-side task record and had no demo fallback path | Added a local prefilled demo draft and made `demo-task-id` load and persist through the browser demo store instead of the backend task API | Public demo entry points should not depend on mutable backend seed data unless that seed is created automatically during deploy |
| 2026-05-24 | Deployment handoff | Deployment knowledge was spread across chat history, making post-launch checks and routine maintenance harder to repeat reliably | The repository had deployment mechanics documented, but no concise end-state checklist or operations memo for the live environment | Added `docs/ACCEPTANCE_CHECKLIST.md` and `docs/OPERATIONS_RUNBOOK.md` covering verification steps, Nginx notes, HTTPS certificate facts, and recurring commands | After production rollout, capture both acceptance criteria and recurring operations in dedicated docs instead of leaving them implicit in conversation history |
...
- Restored the homepage demo-case path by making `demo-task-id` resolve to a built-in local sample draft instead of an assumed backend record.
- Added a final acceptance checklist and an operations runbook for the live Alibaba Cloud deployment.
